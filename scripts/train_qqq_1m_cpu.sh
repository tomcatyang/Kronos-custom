...
# Usage:
#   bash scripts/train_qqq_1m_cpu.sh
#     Train both tokenizer and basemodel with the QQQ 1-minute CPU config.
#
#   bash scripts/train_qqq_1m_cpu.sh --skip-basemodel
#     Train tokenizer only.
#
#   bash scripts/train_qqq_1m_cpu.sh --skip-tokenizer
#     Train basemodel only. Requires an existing fine-tuned tokenizer path.
#
#   bash scripts/train_qqq_1m_cpu.sh --skip-existing
...
.venv/bin/python finetune_csv/train_sequential.py \
  --config finetune_csv/configs/config_qqq_1m_cpu.yaml \
  "$@"
