...
# Usage:
#   bash scripts/train_qqq_5m_cpu.sh
#     Train both tokenizer and basemodel with the QQQ 5-minute CPU config.
#
#   bash scripts/train_qqq_5m_cpu.sh --skip-basemodel
#     Train tokenizer only.
#
#   bash scripts/train_qqq_5m_cpu.sh --skip-tokenizer
#     Train basemodel only. Requires an existing fine-tuned tokenizer path.
#
#   bash scripts/train_qqq_5m_cpu.sh --skip-existing
